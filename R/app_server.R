...
app_server <- function(input, output, session) {
  ...
  # })
  param_list<-eventReactive(input$load,{
    withProgress(message = "Connect to server",value = 0.2,{
      rgee::ee_Initialize(user = 'reto.spielhofer@nina.no')
      incProgress(amount = 0.4,message = "Collect your data")
      user_conf<-user_conf()
      req(studies)
      studies<-studies()
      ...
      incProgress(amount = 0.6,message = "download site geometries")
      ...
      incProgress(amount = 0.8,message = "prepare geodata")
    ...
    list(user_id, userES,site_type,sf_bound,coords,comb,bands,num_tabs)
...
### create N remapping tabs
  ...
  observeEvent(input$tabs_content, {
    num_tabs<-param_list()[[8]]
    userES<-param_list()[[2]]
    sf_bound<-param_list()[[4]]
    comb<-param_list()[[6]]
    bands<-param_list()[[7]]
    userID<-param_list()[[1]]
    site_type<-param_list()[[3]]
    coords<-param_list()[[5]]
